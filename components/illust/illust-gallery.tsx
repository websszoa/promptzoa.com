...
import illustImages from "@/data/illust-image.json";
...
export function IllustGallery() {
  const [visibleCount, setVisibleCount] = useState(BATCH_SIZE);
  const loadMoreRef = useRef<HTMLDivElement>(null);
  const hasMore = visibleCount < illustImages.length;
      ...
      ([entry]) => {
        if (entry.isIntersecting) {
          setVisibleCount((count) =>
            Math.min(count + BATCH_SIZE, illustImages.length),
          );
        }
      },
  ...
  const visibleIllusts = illustImages.slice(0, visibleCount);

  return (
    <section id="illust" className="pb-16" aria-labelledby="gallery-title">
